...
# 1. Update DYNAMIC_FORM_CONFIGS
old_configs = """                const DYNAMIC_FORM_CONFIGS = {
...
new_configs = """                const DYNAMIC_FORM_CONFIGS = {
                  ...
                  'equipment_lab_2': { title: 'Лаборатория / Геодезия', addText: 'Добавить прибор', fields: [{ key: 'name', label: 'Название', type: 'text' }, { key: 'type', label: 'Тип', type: 'text' }, { key: 'description', label: 'Описание', type: 'textarea' }] }
                };"""

if old_configs in content:
    content = content.replace(old_configs, new_configs)
else:
    print("Could not find DYNAMIC_FORM_CONFIGS to replace.")


# 2. Pre-fill state in App initialization
old_init = """      if (!parsed.dynamicLists['about_documents'] || parsed.dynamicLists['about_documents'].length === 0) {
...
new_init = """      if (!parsed.dynamicLists['about_documents'] || parsed.dynamicLists['about_documents'].length === 0) {
      ...
      // Auto-migrate hardcoded data to CMS state if empty
...
if old_init in content:
    ...
    print("Could not find App initialization to replace.")

with open('c:/Users/SystemX/Documents/строй/frontend/src/App.jsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Applied dynamic configs and initializers!")
