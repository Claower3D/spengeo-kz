...
# Add state
if "const [activeArticle, setActiveArticle] = useState(null);" not in content:
    content = content.replace("const [activeAdminSection, setActiveAdminSection] = useState('dashboard');", "const [activeAdminSection, setActiveAdminSection] = useState('dashboard');\n  const [activeArticle, setActiveArticle] = useState(null);")

# Update onClick
old_onclick = "onClick={() => logEvent(`Opened article: ${post.title}`)}"
new_onclick = "onClick={() => { logEvent(`Opened article: ${post.title}`); setActiveArticle(post); }}"
content = content.replace(old_onclick, new_onclick)

# Update onClick in the other place (if any, like single quotes)
old_onclick2 = "onClick={() => logEvent('Opened article: ' + post.title)}"
new_onclick2 = "onClick={() => { logEvent('Opened article: ' + post.title); setActiveArticle(post); }}"
content = content.replace(old_onclick2, new_onclick2)

# Insert modal before closing fragment
modal_code = """
...
print("Added modal for reading articles!")
