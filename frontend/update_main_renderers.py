...
accordion_pattern = re.compile(r'(<div className="accordion-container">).*?(</div>\s*</div>\s*</section>)', re.DOTALL)
...
content = accordion_pattern.sub(new_accordion, content)

# 2. Replace the Bento grid content
bento_pattern = re.compile(r'(<div className="service-bento-grid">).*?(</section>)', re.DOTALL)
  ...
  {(adminData.dynamicLists?.['home_services'] || DEFAULT_HOME_SERVICES).map((srv, i) => (
    <div key={i} className={`service-bento-card ${i === 0 ? 'wide' : i === 5 ? 'full' : ''}`} onClick={() => {setActiveServiceTab('geology'); setActivePage('services');}}>
      <div className="service-bento-bg">
        {/* Placeholder images based on index, ideally add image to admin later if needed */}
...
content = bento_pattern.sub(new_bento, content)

# 3. Replace the Founder block
founder_pattern = re.compile(r'(<section className="about-preview">).*?(</section>)', re.DOTALL)
...
content = founder_pattern.sub(new_founder, content)

with open('c:/Users/SystemX/Documents/строй/frontend/src/App.jsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Replaced main page blocks with dynamic mappers!")
